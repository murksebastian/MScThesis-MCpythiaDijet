...
//	argument as random number seed and the second to specify the number of events to generate
	string ofilename = string("trijet_R10m3_alpha1_#") + argv[1] + string(".cmnd");
...
/*	Hist deltaR01("delta R (leading-subleading)",1000,0.,8.);
	Hist deltaR02("delta R (leading-subsubleading)",1000,0.,8.);
	Hist deltaR12("delta R (subleading-subsubleading)",1000,0.,8.);
*/
...
// exception of Chi and deltaR, these are used to apply selection cuts
	...
	Vec4 p0,p1;
/*	double leading_subleading_deltaR;
	double leading_subsubleading_deltaR;
	double subleading_subsubleading_deltaR;
*/
...
/*
			leading_subleading_deltaR = sqrt(pow(abs(slowjet.y(0) - slowjet.y(1)),2.0)+pow(abs(slowjet.phi(0) - slowjet.phi(1)),2.0));
			leading_subsubleading_deltaR = sqrt(pow(abs(slowjet.y(0) - slowjet.y(2)),2.0)+pow(abs(slowjet.phi(0) - slowjet.phi(2)),2.0));
			subleading_subsubleading_deltaR = sqrt(pow(abs(slowjet.y(1) - slowjet.y(2)),2.0)+pow(abs(slowjet.phi(1) - slowjet.phi(2)),2.0));
*/
			p0 = slowjet.p(0);
			p1 = slowjet.p(1)+slowjet.p(2);
			invM = m(p0,p1);
/*
			std::cout << "deltaR(leading-subleading) = " << leading_subleading_deltaR << '\n';
			std::cout << "deltaR(leading-subsubleading) = " << leading_subsubleading_deltaR << '\n';
			std::cout << "deltaR(subleading-subsubleading) = " << subleading_subsubleading_deltaR << '\n';
*/
...
//	angular selection cuts + invariant mass cuts for chi histograms	
		...
		else std::cout << "Threshold conditions for leading and/or sub(sub)leading jet pT not met. Not starting algorithm." << " ----------" << '\n';
/*
...
// print histograms for deltaR
//	cout << deltaR01 << deltaR02 << deltaR12;

//	create a map linking each PYTHIA histogram with a filename
	map<string,Hist> histograms;
	histograms["trijet_invMass_alpha1"] = invariantMass;
	histograms["trijet_leading_pT_alpha1"] = leading_jet_pT;
	histograms["trijet_subleading_pT_alpha1"] = subleading_jet_pT;
	histograms["trijet_subsubleading_pT_alpha1"] = subsubleading_jet_pT;
	histograms["trijet_leading_y_alpha1"] = leading_jet_y;
	histograms["trijet_subleading_y_alpha1"] = subleading_jet_y;
	histograms["trijet_subsubleading_y_alpha1"] = subsubleading_jet_y;
	histograms["trijet_yStar_alpha1"] = yStar;
	histograms["trijet_yBoost_alpha1"] = yBoost;
	histograms["trijet_chi_2500_2800_alpha1"] = chi_2500_2800;
	histograms["trijet_chi_2800_3100_alpha1"] = chi_2800_3100;
	histograms["trijet_chi_3100_3400_alpha1"] = chi_3100_3400;
	histograms["trijet_chi_3400_4000_alpha1"] = chi_3400_4000;
	histograms["trijet_chi_4000_4600_alpha1"] = chi_4000_4600;
	histograms["trijet_chi_4600_5400_alpha1"] = chi_4600_5400;
	histograms["trijet_chi_5400_13000_alpha1"] = chi_5400_13000;
	histograms["trijet_leading_phi_alpha1"] = leading_jet_phi;
	histograms["trijet_subleading_phi_alpha1"] = subleading_jet_phi;
	histograms["trijet_subsubleading_phi_alpha1"] = subsubleading_jet_phi;
/*	histograms["trijet_deltaR(leading-subleading)_alpha1"] = deltaR01;
	histograms["trijet_deltaR(leading-subsubleading)_alpha1"] = deltaR02;
	histograms["trijet_deltaR(subleading-subsubleading)_alpha1"] = deltaR12;
*/
//	iterate through the map elements (by default in alphabetical order) and write histogram data to .txt files
	for (map<string,Hist>::const_iterator itMap=histograms.begin(); itMap!= histograms.end(); ++itMap) {
		const char* filepath = "MCdata/RsepMin10m3/";
