...
def fillRatioDict(binEdges,D1,D2,filetype1,filetype2):
	dr = defaultdict(dict)
	for elmt,filename in enumerate(sorted(gb.iglob(filetype1)),start=1):
		ifile = open(filename)
		splitFilename = filename.split("_")
		dKey1 = splitFilename[0]+"_"+splitFilename[1]+"_"+splitFilename[2]+"_"+splitFilename[3]
		dKey2 = splitFilename[4]
		h1 = D1[dKey1][dKey2]
	for elmt,filename in enumerate(sorted(gb.iglob(filetype2)),start=1):
		ifile = open(filename)
		splitFilename = filename.split("_")
		dKey1 = splitFilename[0]+"_"+splitFilename[1]+"_"+splitFilename[2]+"_"+splitFilename[3]
		dKey2 = splitFilename[4]
		h2 = D2[dKey1][dKey2]
		hr = ROOT.TH1D("ratio hist","hr",len(binEdges)-1,array('f',binEdges))
		for k in range(1,len(binEdges)):
			if (h1.GetBinContent(k) > 0):
				hr.SetBinContent(k,(h1.GetBinContent(k)/h2.GetBinContent(k)))
			else:
				hr.SetBinContent(k,0)
		print "Printout for ratio histogram (weighted/unweighted):"
		hr.Print("all")
...
def drawHist(alphaOrder,c,d,colourN,styleN,L,label):
	for keyN, dKey in enumerate(sorted(d),start=1):
		...
		if ("ratio" not in label):
			...
			ROOT.gPad.SetLogx()
			h.SetTitle("ratio #Delta R = 0.1 / #Delta R = 1.0 trijet #Omicron(#alpha_{s}) = 1 (@LO,weighted)")
			#h.GetXaxis().SetTitle("#chi")
...
DtriR01R10r,DtriR10R01r = (defaultdict(dict) for i in range(2))
...
DtriR01,DtriR01w,DtriR01r = fillDict("RsepMin01m2/trijet_chi*alpha1.txt",chiLogBins,"RsepMin01m2/trijet01R01m2rV.txt")
...
DtriR01R10r = fillRatioDict(chiLogBins,DtriR01w,DtriR10w,"RsepMin01m2/trijet_chi*alpha1.txt","RsepMin10m2/trijet_chi*alpha1.txt")
#DtriR10R01r = fillRatioDict("RsepMin10m2/trijet_chi*alpha1.txt","RsepMin01m2/trijet_chi*alpha1.txt",chiLogBins,DtriR10w,DtriR01w)

L = ROOT.TLegend(0.1,0.1,0.9,0.9)
drawHist(alphaOrder,canv,Ddi,4,33,L,"dijet #Omicron(#alpha_{s}) = 1 (@LO)  ")
drawHist(alphaOrder,canv,DdiNLO,38,27,L,"dijet #Omicron(#alpha_{s}) = 1 (@NLO)  ")
#drawHist(alphaOrder,canv,DtriR01,2,20,L,"trijet alpha_s (order) = 1 (@LO), PhaseSpace:RsepMin = 0.1")
drawHist(alphaOrder,canv,DtriR01w,46,24,L,"trijet #Omicron(#alpha_{s}) = 1 (@LO,weighted), #Delta R = 0.1  ")
...
drawHist(alphaOrder,canv,DtriR10w,28,32,L,"trijet #Omicron(#alpha_{s}) = 1 (@LO,weighted), #Delta R = 1.0  ")
#drawHist(alphaOrder,canv,DtriR10r,1,20,L,"ratio of trijet alpha_s (order) = 1, PhaseSpace:RsepMin = 1.0 (weighted/unweighted)")
#drawHist(alphaOrder,canv,DtriR01R10r,1,20,L,"ratio #Delta R = 0.1 / #Delta R = 1.0 trijet #Omicron(#alpha_{s}) = 1 (@LO,weighted) ")
canv.cd(8)
L.Draw("same")
canv.SaveAs("Results/ChiPlots/Ch5.5.3.2nChi_di1NLO_tri1rR01R10.pdf")
